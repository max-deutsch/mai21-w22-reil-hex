    ...
    for i_game in range(config.num_worker_games):
        ...
        try:
            ...
            while True:
                ...
                num_iterations, mcts_result = mcts.run(game_state=game_state, max_num_iterations=config.num_mcts_iterations,
                                                       device=device, max_seconds=config.max_mcts_time)  # 0.1/num_parallel_mcts)
            ...
            tmp_mcts_values = np.asarray(tmp_mcts_values_override) # can be switched to normal
            ...
            np.savetxt("tmp_data/" + str(worker_id) + "_" + str(i_game) + "_board.txt",tmp_mcts_boards.reshape(tmp_mcts_boards.shape[0], -1))
